...
namespace AnatomyLabeller
{
	public partial class CardCreator : Form
	{
		...
		private MouseEventArgs mouseEventForFirstClick;

		public FlashCard newFlashCard;

		private const string addLabelText = "Add Label";

		private const string editLabelText = "Edit Label";
		...
		private Point originalLabelPosition;

		//The position that the image is drawn at
		private Point imagePosition = new Point ( 416 , 32 );

		private Image image;

		private Point labelMousePosition;
		...
		private void btnOK_Click ( object sender , EventArgs e )
		{
			...
			if ( newFlashCard.Labels.GetLength ( 0 ) == 0 )
			{
			...
			newFlashCard.CardName = txtCardName.Text;
		...
		private void CardCreator_Paint ( object sender , PaintEventArgs e )
		{
			...
			//Draw the labels
			Label [ ] labels = newFlashCard.Labels;
			...
			foreach ( Label label in labels )
			{
				...
				DrawLabel ( e , drawRect , label.ID.ToString ( ) );
			}
		}
		...
		private void btnAddLabel_Click ( object sender , EventArgs e )
		{
			...
			if ( btnAddLabel.Text == addLabelText )
			{
			...
			else if ( btnAddLabel.Text == editLabelText )
			{
				if ( currentLabel != null )
				{
					newFlashCard.RemoveLabel ( currentLabel );

					currentLabel.Name = txtLabelName.Text;

					currentLabel.ID = ( int ) numLabelID.Value;

					newFlashCard.AddLabel ( currentLabel );

					ResetCurrentLabel ( true );

					btnAddLabel.Text = addLabelText;
					...
					case State.Clear:
						...
						if ( currentLabel == null )
						{
							...
							foreach ( Label label in labels )
							{
								...
								{
									//Get the label
									originalLabelPosition = label.Origin;
									currentLabel = label;
									lblCurrentLabelName.Text = currentLabel.Name;
		...
		private void ProcessSingleClick ( MouseEventArgs e )
		{
			if ( ClickedWithinImage ( e.X , e.Y ) )
			{
				switch ( currentState )
						...
						if ( currentLabel == null )
						{
							...
							foreach ( Label label in labels )
							{
								if ( label.MouseClick ( new Point ( e.X - imagePosition.X , e.Y - imagePosition.Y ) ) )
								{
									...
									txtLabelName.Text = currentLabel.Name;
