...
namespace AnatomyLabeller
{
	public partial class frmMain : Form
	{
		...
		private Rectangle imageRect = new Rectangle ( 303 , 26 , 753 , 597 );
		...
		private List<AnswerState> labelAnswerStates;
		...
		private void frmMain_Paint ( object sender , PaintEventArgs e )
		{
			...
			{
				//Draw the labels
				Label [ ] labels = currentCard.Labels;
				...
				for ( int i = 0 ; i < labels.Length ; i++ )
				{
					...
					DrawLabel ( e , drawRect , labels [ i ].ID.ToString ( ) , labelAnswerStates [ i ] );
				}
			}
		}
		...
		private void lstCardList_SelectedIndexChanged ( object sender , EventArgs e )
		{
			if ( lstCardList.SelectedIndex >= 0 && lstCardList.SelectedIndex < lstCardList.Items.Count )
			{
				...
				currentImage = Image.FromFile ( currentCard.ImageFilepath );

				imageRect.Size = currentCard.ImageSize;

				labelAnswerStates = new List<AnswerState> ( );

				for ( int i = 0 ; i < currentCard.Labels.Length ; i++ )
				{
		...
		private void SaveFile ( string filePath )
		{
			...
			foreach ( FlashCard card in lstCardList.Items )
			{
				...
				xmlWriter.WriteAttributeString ( "CardName" , card.CardName );

				xmlWriter.WriteAttributeString ( "Image" , card.ImageFilepath );

				xmlWriter.WriteStartAttribute ( "ImageSizeW" );
				xmlWriter.WriteValue ( card.ImageSize.Width );
				xmlWriter.WriteEndAttribute ( );

				xmlWriter.WriteStartAttribute ( "ImageSizeH" );
				xmlWriter.WriteValue ( card.ImageSize.Height );
				xmlWriter.WriteEndAttribute ( );
				...
				foreach ( Label label in card.Labels )
				{
					...
					xmlWriter.WriteValue ( label.ID );
					xmlWriter.WriteEndAttribute ( );

					xmlWriter.WriteAttributeString ( "Name" , label.Name );

					xmlWriter.WriteStartAttribute ( "OriginX" );
					xmlWriter.WriteValue ( label.Origin.X );
					xmlWriter.WriteEndAttribute ( );

					xmlWriter.WriteStartAttribute ( "OriginY" );
					xmlWriter.WriteValue ( label.Origin.Y );
					xmlWriter.WriteEndAttribute ( );
				...
				while ( reader.Read ( ) )
				{
					if ( reader.NodeType == XmlNodeType.Element )
					{
						if ( reader.Name == "FlashCard" )
						{
							...
							flashCard.CardName = reader.GetAttribute ( "CardName" );
						...
						if ( reader.Name == "Label" )
						{
							...
							label.ID = Convert.ToInt32 ( reader.GetAttribute ( "ID" ) );
					...
					for ( int i = 0 ; i < labels.Length ; i++ )
					{
						if ( labels [ i ].MouseClick ( new Point ( e.X - imageRect.X , e.Y - imageRect.Y ) ) )
						{
							if ( labelAnswerStates [ i ] == AnswerState.None )
							{
								...
								lblLabelID.Text = "Label " + currentLabel.ID + ":";
		...
		{
			if ( currentLabel.Name.ToUpper ( ) == txtAnswer.Text.ToUpper ( ) )
			{
			...
			lblCorrectAnswer.Text = "Correct Answer: " + currentLabel.Name;
		...
		private void ResetAnswers ( )
		{
			...
			{
				MessageBox.Show ( "No Labels Found","Error " );
			}
		}
		...
		private void lstCardList_KeyDown ( object sender , KeyEventArgs e )
		{
			if ( lstCardList.SelectedIndex >= 0 )
			{
				if ( e.KeyCode == Keys.Delete )
				{
					...
					if ( card != null )
					{
						if ( MessageBox.Show ( "Are you sure you want to delete " + card.CardName + "?" ,
